...
riot.tag2('grid2', '<div riot-style="height:{opts.height}px" class="gridwrap"> <div onscroll="{scrolling}" riot-style="overflow:auto;left:{fixedLeftWidth}px;top:{rowHeight}px;bottom:0px" class="gridbody"> <div riot-style="width:{scrollWidth-fixedLeftWidth}px;height:{scrollHeight-rowHeight}px" class="scrollArea"> <div each="{visCells.main}" no-reorder riot-style="transform: translate3d({left}px,{top}px,0px); backface-visibility: hidden;width:{width}px;height:{rowHeight}px;" class="cell">{text}</div> </div> </div> <div riot-style="height:{rowHeight}px" class="gridbody"> <div riot-style="top:0px;left:0px;width:{scrollWidth}px;height:{rowHeight}px" class="header"> <div each="{headers.main}" no-reorder riot-style="transform:translate3d({left}px,0px,0px); backface-visibility: hidden;width:{width}px;height:{rowHeight}px;" class="headercell">{text}</div> </div> </div> <div riot-style="width:{fixedLeftWidth}px;height:{opts.height-2}px" class="gridbody"> <div riot-style="left:0px;top:{0-gridbody[0].scrollTop}px;width:{fixedLeftWidth}px;bottom:1px;z-index:2;" class="fixedLeft"> <div riot-style="top:{gridbody[0].scrollTop}px;left:0px;width:{fixedLeftWidth}px;height:{rowHeight}px" class="header"> <div each="{headers.fixed}" riot-style="top:0px;left:{left}px;width:{width}px;height:{rowHeight}px;" class="headercell">{text}</div> </div> <div each="{visCells.fixed}" no-reorder riot-style="transform:translate3d({left}px,{top}px,0px);backface-visibility: hidden;width:{width}px;height:{rowHeight}px;" class="cell">{text} </div> </div> </div> </div>', 'grid2 { display: block; } grid2 .gridwrap { position: relative; display: block; border: 1px solid #ccc; font-family: sans-serif; font-size: 14px; } grid2 .gridbody { position: absolute; overflow: hidden; top: 0; left: 0; right: 0; bottom: 0; } grid2 .fixedLeft { position: absolute; top: 0; bottom: 0; } grid2 .cell, grid2 .headercell { position: absolute; -webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box; padding: 5px; white-space: nowrap; overflow: hidden; background: #fff; border: 1px solid #ccc; border-width: 0 1px 1px 0; } grid2 .header { position: absolute; z-index: 1; overflow: hidden; }', '', function(opts) {
...
this.on('update', function() {
  ...
  if (!this.visCells) {
    ...
    this.visCells = calcVisible(this.rows, this.gridbody[0], this.rowHeight);
    return console.timeEnd('calcvis');
  } else {
    console.time('recalc');
    this.visCells = reCalc(this.visCells, this.rows, this.gridbody[0], this.rowHeight);
    return console.timeEnd('recalc');
  }
});
...
this.scrolling = (function(_this) {
  return function(e) {
    ...
    _this.gridbody[1].scrollLeft = _this.gridbody[0].scrollLeft;
    return _this.update();
  };
})(this);
