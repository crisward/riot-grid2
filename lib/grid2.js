...
riot.tag2('grid2', '<div riot-style="height:{opts.height}px" class="gridwrap"> <div ref="mainbody" riot-style="left:{fixedLeft.width}px;top:{rowHeight}px;bottom:0px" class="gridbody"> <div riot-style="transform:translate3d({fixedLeft.left}px,{fixedLeft.top}px,0px);backface-visibility: hidden;width:{fixedLeft.width}px;bottom:1px;z-index:2;" class="fixedLeft"> <div data-is="{cell.tag || \'gridcelltag\'}" data="{parent.opts.data}" val="{cell.text}" cell="{cell}" each="{cell in visCells.main}" onclick="{handleClick}" riot-style="position: absolute;left:{cell.left}px;top:{cell.top}px;width:{cell.width}px;height:{rowHeight}px;" class="cell {active:cell.active}"></div> </div> </div> <div ref="header" riot-style="height:{rowHeight}px;margin-right:15px" class="gridbody"> <div riot-style="top:0px;left:0px;width:{scrollWidth}px;height:{rowHeight}px" class="header"> <div each="{headers.main}" riot-style="transform:translate3d({left}px,0px,0px); backface-visibility: hidden;width:{width}px;height:{rowHeight}px;" class="headercell">{text}</div> </div> </div> <div riot-style="width:{fixedLeft.width}px;height:{opts.height-2}px" class="gridbody"> <div riot-style="transform:translate3d(0px,{fixedLeft.top}px,0px);backface-visibility: hidden;width:{fixedLeft.width}px;bottom:1px;z-index:2;" class="fixedLeft"> <div riot-style="top:{0 - fixedLeft.top}px;left:0px;width:{fixedLeft.width}px;height:{rowHeight}px" class="header"> <div each="{headers.fixed}" riot-style="top:0px;left:{left}px;width:{width}px;height:{rowHeight}px;" class="headercell">{text}</div> </div> <div data-is="{cell.tag || \'gridcelltag\'}" data="{parent.opts.data}" val="{cell.text}" cell="{cell}" each="{cell in visCells.fixed}" onclick="{handleClick}" riot-style="position: absolute;left:{cell.left}px;top:{cell.top}px;width:{cell.width}px;height:{rowHeight}px;" class="cell {active:cell.active}"></div> </div> </div> <div ref="overlay" onscroll="{scrolling}" riot-style="overflow:auto;left:0px;top:{rowHeight}px;bottom:0px;-webkit-overflow-scrolling: touch;" class="gridbody"> <div riot-style="background:rgba(0,0,0,0.005);width:{scrollWidth}px;height:{scrollHeight-rowHeight}px;" class="scrollArea"></div> </div> </div>', 'grid2 { display: block; -webkit-font-smoothing: antialiased; text-rendering: optimizeSpeed; } grid2 .scrollArea { transform: translateZ(0); } grid2 .gridwrap { position: relative; display: block; border: 1px solid #ccc; font-family: sans-serif; font-size: 14px; } grid2 .gridbody { position: absolute; overflow: hidden; top: 0; left: 0; right: 0; bottom: 0; transform: translateZ(0); backface-visibility: hidden; } grid2 .fixedLeft { position: absolute; top: 0; bottom: 0; } grid2 .cell, grid2 .headercell { position: absolute; box-sizing: border-box; padding: 10px 5px; whitespace: no-wrap; overflow: hidden; background: #fff; border: 1px solid #eee; border-width: 0 1px 1px 0; cursor: pointer; } grid2 .cell.active { background: #eee; } grid2 .header { position: absolute; z-index: 1; overflow: hidden; transform: translateZ(0); }', '', function(opts) {
...
this.on('mount', function() {
  ...
  this.refs.overlay.addEventListener('click', this.pushThroughClick);
  this.refs.overlay.addEventListener('dlbclick', this.pushThroughClick);
  return this.update();
});

this.on('before-unmount', function() {
  this.refs.overlay.removeEventListener('click', this.pushThroughClick);
  return this.refs.overlay.removeEventListener('dlbclick', this.pushThroughClick);
});
...
this.on('update', function() {
  ...
  } else if (this.visCells.main.length === 0) {
    return this.visCells = calcVisible(this.rows, this.refs.overlay, this.rowHeight);
  } else {
...
riot.tag2('gridcelltag', '{opts.val}', '', '', function(opts) {
});